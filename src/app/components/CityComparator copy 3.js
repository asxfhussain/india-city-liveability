...
export default function CityComparator() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29, #302b63, #24243e)", padding: "2rem 1rem", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, overflow: "hidden", pointerEvents: "none", zIndex: 0 }}>
        <motion.div animate={{ x: [0, 30, 0], y: [0, -20, 0] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          style={{ position: "absolute", top: "10%", left: "15%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,102,241,0.15), transparent 70%)" }} />
        <motion.div animate={{ x: [0, -20, 0], y: [0, 30, 0] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
          style={{ position: "absolute", top: "50%", right: "10%", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(34,211,238,0.1), transparent 70%)" }} />
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 6 }}>
            <motion.div whileHover={{ rotate: 10, scale: 1.1 }} style={{ width: 48, height: 48, borderRadius: 14, background: "linear-gradient(135deg, #6366f1, #22d3ee)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, boxShadow: "0 8px 24px rgba(99,102,241,0.4)" }}>
              🏙️
            </motion.div>
            <div>
              <h1 style={{ fontSize: 28, fontWeight: 800, color: "#fff", margin: 0, letterSpacing: -1 }}>India City Liveability</h1>
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", margin: 0 }}>Real-time · 15 cities · Live AQI & Weather</p>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", borderRadius: 20, padding: "1.25rem 1.5rem", border: "1px solid rgba(255,255,255,0.1)", marginBottom: "1.25rem" }}>
          <p style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.35)", letterSpacing: 2, marginBottom: 14 }}>SELECT UP TO 4 CITIES</p>
            ...
            {CITIES.map((c, i) => (
              <motion.button key={c.id} onClick={() => toggleCity(c.id)}
                ...
                style={{
                  ...
                  color: selected.includes(c.id) ? "#fff" : "rgba(255,255,255,0.55)",
                  boxShadow: selected.includes(c.id) ? "0 4px 20px rgba(99,102,241,0.5)" : "none",
                }}>
                {c.name}
              </motion.button>
            ))}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
          style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", borderRadius: 20, padding: "1.25rem 1.5rem", border: "1px solid rgba(255,255,255,0.1)", marginBottom: "1.25rem" }}>
          <p style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.35)", letterSpacing: 2, marginBottom: 16 }}>ADJUST YOUR PRIORITIES</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 20 }}>
            {WEIGHTS.map((w) => (
              <div key={w.key}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 12, color: "rgba(255,255,255,0.55)" }}>{w.icon} {w.label}</span>
                  <motion.span key={weights[w.key]} initial={{ scale: 1.3, color: "#22d3ee" }} animate={{ scale: 1 }} style={{ fontSize: 12, fontWeight: 700, color: "#22d3ee" }}>
        ...
        <AnimatePresence mode="wait">
          {topCity && (
            <motion.div key={topCity.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.4 }}
              style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.25), rgba(34,211,238,0.15))", borderRadius: 16, padding: "14px 20px", border: "1px solid rgba(99,102,241,0.35)", marginBottom: "1.25rem", display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <span style={{ color: "#fff", fontSize: 14 }}>
                <strong style={{ color: "#22d3ee" }}>{topCity.name}</strong> is your best match with a score of{" "}
                <strong style={{ color: "#22d3ee" }}>{calcScore(topCity, weights, liveWeather, liveAqi)}/100</strong> based on your priorities
              </span>
              {!loaded && <span style={{ marginLeft: "auto", fontSize: 12, color: "rgba(255,255,255,0.4)" }}>⟳ Loading live data...</span>}
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <AnimatePresence>
            {selectedCities.map((c, i) => (
              <motion.div key={c.id}
                ...
                style={{
                  background: i === 0 ? "linear-gradient(135deg, rgba(99,102,241,0.2), rgba(34,211,238,0.1))" : "rgba(255,255,255,0.05)",
                ...
                {i === 0 && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    style={{ display: "inline-block", fontSize: 11, fontWeight: 700, background: "linear-gradient(135deg, #6366f1, #22d3ee)", color: "#fff", padding: "3px 12px", borderRadius: 50, marginBottom: 10, letterSpacing: 0.5 }}>
                    ✦ BEST MATCH
                  </motion.div>
                )}
                  ...
                  return (
                    <div key={w.key} style={{ marginBottom: 9 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.45)" }}>{w.icon} {w.label}</span>
                        <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.75)" }}>{label}</span>
                      </div>
                      <div style={{ height: 5, background: "rgba(255,255,255,0.08)", borderRadius: 5 }}>
                        <motion.div initial={{ width: 0 }} animate={{ width: `${val}%` }} transition={{ duration: 0.8, delay: i * 0.1, ease: "easeOut" }}
                          style={{ height: 5, borderRadius: 5, background: `linear-gradient(90deg, ${COLORS[i]}, ${COLORS[i]}99)` }} />
                      </div>
                    </div>
                  );
                })}
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 12, lineHeight: 1.7 }}>{c.note}</p>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4 }}
          style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", borderRadius: 20, padding: "1.5rem", border: "1px solid rgba(255,255,255,0.08)" }}>
          <p style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.6)", marginBottom: 16 }}>Metric Comparison</p>
          <ResponsiveContainer width="100%" height={320}>
            <RadarChart data={radarData}>
              <PolarGrid stroke="rgba(255,255,255,0.08)" />
              <PolarAngleAxis dataKey="metric" tick={{ fontSize: 12, fill: "rgba(255,255,255,0.45)" }} />
              {selectedCities.map((c, i) => (
